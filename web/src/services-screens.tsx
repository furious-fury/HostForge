...
export function ServicesList({ applicationID }: { applicationID: string }) {
  ...
  const [category, setCategory] = useState<"active" | "deleted">("active")
  ...
  const activeServices = services.filter((service) => !deletedServiceIDs.has(service.id))
  const categoryServices = category === "deleted" ? services.filter((service) => deletedServiceIDs.has(service.id)) : activeServices
