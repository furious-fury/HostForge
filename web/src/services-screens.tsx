...
export function AddService({ applicationID }: { applicationID: string }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (service) => navigate("/applications/" + applicationID + "/services/" + service.id),
  })
  ...
  return <main className="mx-auto w-full max-w-5xl px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    ...
    <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); createMutation.mutate() }}>
      <Panel title="GitHub source" subtitle="HostForge uses installation tokens; PAT and SSH credentials are not accepted"><div className="grid gap-5 p-6 sm:grid-cols-2"><Field label="Installation"><AppSelect options={installations.map((item) => item.account_login)} value={installation?.account_login || installationName} onValueChange={(value) => { setInstallationName(value); setRepositoryName(""); setBranch("") }} className="h-10 w-full bg-background text-xs" /></Field><Field label="Repository"><AppSelect options={repositories.map((item) => item.full_name)} value={repository?.full_name || repositoryName} onValueChange={(value) => { setRepositoryName(value); setBranch("") }} disabled={!installation || repositoriesQuery.isPending} className="h-10 w-full bg-background text-xs" /></Field><Field label="Environment"><AppSelect options={environments.map((item) => item.name)} value={environment?.name || environmentName} onValueChange={setEnvironmentName} className="h-10 w-full bg-background text-xs" /></Field><Field label="Branch"><AppSelect options={branches} value={selectedBranch} onValueChange={setBranch} disabled={!repository || branchesQuery.isPending} className="h-10 w-full bg-background text-xs" /></Field></div>{installationsQuery.isError && <p className="border-t px-6 py-3 text-xs text-destructive">GitHub installations could not be loaded. Complete GitHub App setup first.</p>}</Panel>
      ...
      {createMutation.isError && <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-xs text-destructive">{createMutation.error.message}</div>}
...
export function ServiceOverview({ applicationID, service: serviceID }: { applicationID: string; service: string }) {
  ...
  return <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    ...
    <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end"><div className="flex items-start gap-4"><span className="grid size-12 place-items-center rounded-xl bg-accent text-accent-foreground"><CubeIcon size={22} weight="fill" /></span><div><p className="mb-1"><StatusPill status={state} /></p><h1 className="text-3xl font-semibold tracking-[-0.035em]">{service.name}</h1><p className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground"><span className="flex items-center gap-1"><GithubLogoIcon size={13} />{service.repo_url}</span><span className="flex items-center gap-1"><GitBranchIcon size={13} />{binding?.branch || "No branch selected"}</span></p></div></div>
      <div className="flex flex-wrap gap-2 xl:ml-auto"><AppSelect options={environments.map((item) => item.name)} value={environment?.name || environmentName} onValueChange={setEnvironmentName} className="h-9 min-w-36 bg-card text-xs" />{binding?.desired_state !== "stopped" && binding?.active_deployment_id && <ConfirmationAction title="Stop this service?" description="The active container will stop until it is restarted or redeployed." confirmLabel="Stop service" onConfirm={() => stopMutation.mutateAsync()} trigger={<Button variant="outline" disabled={stopMutation.isPending}><PauseIcon />Stop</Button>} />}{binding?.active_deployment_id && <Button variant="outline" disabled={restartMutation.isPending} onClick={() => restartMutation.mutate()}><ActivityIcon />Restart</Button>}<Button disabled={!binding?.branch || deployMutation.isPending} onClick={() => deployMutation.mutate()}><RocketLaunchIcon weight="fill" />Deploy</Button></div>
    </div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Panel title="Latest deployment" subtitle={latest ? new Date(latest.created_at).toLocaleString() : "No deployment recorded"} action={latest ? <Link to={"/deployments/" + latest.id} className="text-xs font-medium hover:underline">View deployment</Link> : undefined}>{deploymentsQuery.isPending ? <div className="h-36 animate-pulse bg-muted/40" /> : latest ? <div className="p-5"><div className="flex items-center gap-2"><StatusPill status={latest.status === "SUCCESS" ? "Healthy" : latest.status[0] + latest.status.slice(1).toLowerCase()} /><span className="font-mono text-xs">{latest.id}</span></div><p className="mt-4 font-mono text-xs text-muted-foreground">{latest.commit_hash || "Commit pending"}</p><p className="mt-2 text-xs text-muted-foreground">{latest.trigger || "manual"} / {latest.actor || "operator"}</p></div> : <div className="p-8 text-center text-xs text-muted-foreground">Deploy this environment to create its first release.</div>}</Panel>
      <Panel title="Domains" subtitle="Routes for this environment" action={<Link to={base + "/domains"} className="text-xs font-medium hover:underline">Manage</Link>}>{domainsQuery.isPending ? <div className="h-36 animate-pulse bg-muted/40" /> : domains.length ? <div className="divide-y">{domains.map((domain) => <div key={domain.id} className="flex items-center gap-3 px-5 py-4"><GlobeIcon size={15} className="text-muted-foreground" /><span className="text-xs font-medium">{domain.domain_name}</span><DomainStatus value={domain.ssl_status} /></div>)}</div> : <div className="p-8 text-center text-xs text-muted-foreground">No public routes configured.</div>}</Panel>
    </div>
  </main>
}
