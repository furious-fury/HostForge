...
export function ServicesList({ applicationID }: { applicationID: string }) {
  ...
  const [category, setCategory] = useState<"active" | "failed" | "deleted">("active")
  ...
  const serviceStates = new Map(services.map((service) => [service.id, service.service_type === "database" ? databaseServiceListState(databaseInstances[service.id] || [], environments) : serviceListState(bindings[service.id] || [], environments)]))
  const deletedServiceIDs = new Set(services.filter((service) => { const instances = databaseInstances[service.id] || []; return service.service_type === "database" && instances.length > 0 && instances.every((instance) => instance.status === "deleted") }).map((service) => service.id))
  const failedServiceIDs = new Set(services.filter((service) => !deletedServiceIDs.has(service.id) && serviceStates.get(service.id)?.status === "Failed").map((service) => service.id))
  const activeServices = services.filter((service) => !deletedServiceIDs.has(service.id) && !failedServiceIDs.has(service.id))
  const categoryServices = category === "deleted" ? services.filter((service) => deletedServiceIDs.has(service.id)) : category === "failed" ? services.filter((service) => failedServiceIDs.has(service.id)) : activeServices
  const visibleRows = categoryServices.filter((service) => [service.name, service.repo_url, service.runtime].join(" ").toLowerCase().includes(query.toLowerCase()))
  const states = activeServices.map((service) => service.service_type === "database" ? databaseServiceListState(databaseInstances[service.id] || [], environments) : serviceListState(bindings[service.id] || [], environments))
...
export function ServiceOverview({ applicationID, service: serviceID }: { applicationID: string; service: string }) {
  ...
  return <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end"><div className="flex items-start gap-4"><StackIdentity kind={service.stack_kind} label={service.stack_label} showLabel={false} iconClassName="size-12 rounded-xl bg-accent text-accent-foreground" /><div><div className="mb-1"><StatusPill status={state} /></div><h1 className="text-3xl font-semibold tracking-[-0.035em]">{service.name}</h1><div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground"><span className="flex items-center gap-1"><GithubLogoIcon size={13} />{service.repo_url}</span><StackIdentity kind={service.stack_kind} label={service.stack_label} iconClassName="size-6 rounded-md" /><span>{activeEnvironments.length} active {activeEnvironments.length === 1 ? "environment" : "environments"}</span></div></div></div>
