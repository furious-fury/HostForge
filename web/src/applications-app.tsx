  ...
  return (
    <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
      ...
      <section className="overflow-hidden rounded-xl border bg-card">
        ...
        {services.length ? <div className="divide-y">{services.map((service) => <Link key={service.id} to={base + "/services/" + service.id} onPointerEnter={() => queryClient.prefetchQuery({ queryKey: queryKeys.service(service.id), queryFn: ({ signal }) => api.service(service.id, signal) })} onFocus={() => queryClient.prefetchQuery({ queryKey: queryKeys.service(service.id), queryFn: ({ signal }) => api.service(service.id, signal) })} className="flex items-center gap-3 px-5 py-4 hover:bg-muted/35">{service.service_type === "database" ? <DatabaseIdentity engine={service.stack_kind} label={service.stack_label} showLabel={false} /> : <StackIdentity kind={service.stack_kind} label={service.stack_label} showLabel={false} />}<span className="min-w-0"><span className="block truncate text-xs font-semibold">{service.name}</span><span className="mt-1 block truncate text-[11px] text-muted-foreground">{service.service_type === "database" ? "HostForge managed database" : service.repo_url}</span><span className="mt-1 block truncate text-[10px] font-medium text-muted-foreground">{service.stack_label || service.stack_kind || "Stack not detected yet"}</span></span><span className="ml-auto font-mono text-[10px] text-muted-foreground">:{service.internal_port}</span></Link>)}</div> : <div className="px-6 py-14 text-center"><CubeIcon className="mx-auto text-muted-foreground" size={24} /><p className="mt-3 text-sm font-semibold">No services yet</p><p className="mt-1 text-xs text-muted-foreground">Connect a repository to create the first deployable service.</p><Button className="mt-4" onClick={() => navigate(base + "/services/new")}><PlusIcon /> Add service</Button></div>}
      </section>
    </main>
  )
}
