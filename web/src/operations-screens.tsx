...
export function ServiceMetrics({ applicationID, service }: { applicationID: string; service: string }) {
  ...
  if (!metricsQuery.data.supported) return <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9"><PageHeader title="Metrics" description="Container resource samples for this service." back={{ label: "Service overview", href: base }}><AppSelect options={environments.map((item) => item.name)} value={environment?.name || environmentName} onValueChange={setEnvironmentName} /></PageHeader><ServiceTabs active="Metrics" serviceID={service} applicationID={applicationID} /><StateCard title="No active container" description={metricsQuery.data.error_code || "Deploy this environment before collecting service metrics."} /></main>
...
export function EnvironmentScreen({ scope, applicationID, service = "" }: { scope: "application" | "service"; applicationID: string; service?: string }) {
  ...
  return <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    ...
    {scope === "application" ? <ApplicationTabs active="Environment" applicationID={applicationID} /> : <ServiceTabs active="Environment" serviceID={service} applicationID={applicationID} />}
...
export function DomainsScreen({ scope, applicationID, service = "" }: { scope: "application" | "service"; applicationID: string; service?: string }) {
  ...
  return <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    ...
    {scope === "application" ? <ApplicationTabs active="Domains" applicationID={applicationID} /> : <ServiceTabs active="Domains" serviceID={service} applicationID={applicationID} />}
