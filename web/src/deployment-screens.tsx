...
export function DeploymentsList({ scope = "global", service = "", applicationID = "" }: { scope?: "global" | "application" | "service"; service?: string; applicationID?: string }) {
  ...
  const [applicationFilter, setApplicationFilter] = useState(scope === "global" ? all : applicationID)
  const [serviceFilter, setServiceFilter] = useState(scope === "service" ? service : all)
  ...
  const selectedApplicationID = scope === "global" ? applicationFilter === all ? "" : applicationFilter : applicationID
  const selectedServiceID = scope === "service" ? service : serviceFilter === all ? "" : serviceFilter
  const selectedEnvironmentID = environmentFilter === all ? "" : environmentFilter
  const applicationsQuery = useQuery({ queryKey: queryKeys.applications, queryFn: ({ signal }) => api.applications(signal), enabled: scope === "global" })
  const applicationQuery = useQuery({ queryKey: queryKeys.application(selectedApplicationID), queryFn: ({ signal }) => api.application(selectedApplicationID, signal), enabled: Boolean(selectedApplicationID) })
  const deploymentFilters = {
    applicationID: selectedApplicationID || undefined,
    serviceID: selectedServiceID || undefined,
    environmentID: selectedEnvironmentID || undefined,
    status: status === "All statuses" ? undefined : status.toUpperCase(),
    trigger: trigger === "All triggers" ? undefined : trigger.toLowerCase(),
  ...
  const deploymentsQuery = useQuery({ queryKey: queryKeys.deploymentList(deploymentFilters), queryFn: ({ signal }) => api.deployments(deploymentFilters, signal), refetchInterval: 5000 })
  const resetPage = () => { setCursor(""); setCursorHistory([]) }
  const services = applicationQuery.data?.services || []
  const environments = applicationQuery.data?.environments || []
  const advancedCount = (scope === "global" && selectedApplicationID ? 1 : 0) + (scope !== "service" && selectedServiceID ? 1 : 0) + (selectedEnvironmentID ? 1 : 0) + (branch.trim() ? 1 : 0) + (dateFrom ? 1 : 0) + (dateTo ? 1 : 0)
  const clearAdvanced = () => {
    if (scope === "global") setApplicationFilter(all)
    if (scope !== "service") setServiceFilter(all)
  ...
  if (deploymentsQuery.isPending || scope !== "global" && applicationQuery.isPending) return <main className="mx-auto w-full max-w-[1600px] animate-pulse px-4 py-8 sm:px-6 lg:px-8"><div className="h-8 w-48 rounded bg-muted" /><div className="mt-6 h-80 rounded-xl border bg-card" /></main>
  if (deploymentsQuery.isError || scope !== "global" && applicationQuery.isError) return <main className="mx-auto w-full max-w-[1600px] px-4 py-16 sm:px-6 lg:px-8"><section className="rounded-xl border bg-card p-8 text-center"><h1 className="text-sm font-semibold">Deployments could not be loaded</h1><p className="mt-2 text-xs text-muted-foreground">The server did not return the required application and deployment history.</p><Button className="mt-4" variant="outline" onClick={() => { deploymentsQuery.refetch(); if (selectedApplicationID) applicationQuery.refetch() }}>Retry</Button></section></main>
  ...
  return (
    <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
      <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end"><div><h1 className="text-3xl font-semibold tracking-[-0.035em]">Deployments</h1><p className="mt-2 text-sm text-muted-foreground">{scope === "global" ? "All deployment activity across every application, service, and environment." : "Deployment activity for this scope."}</p></div><Button className="sm:ml-auto" variant="outline" onClick={() => deploymentsQuery.refetch()}><ArrowsClockwiseIcon />Refresh</Button></div>
      {scope === "application" && <ApplicationTabs active="Deployments" applicationID={applicationID} />}
      {scope === "service" && <ServiceTabs active="Deployments" serviceID={service} applicationID={applicationID} />}
      ...
      <section className="overflow-hidden rounded-xl border bg-card">
        <header className="grid gap-3 border-b bg-muted/70 p-3 sm:grid-cols-[9rem_9rem_auto_minmax(14rem,1fr)]"><AppSelect options={["All statuses", "Queued", "Building", "Success", "Failed", "Cancelled"]} value={status} onValueChange={(value) => { setStatus(value); resetPage() }} className="h-[2.1rem] min-w-0 bg-card px-2.5 text-[0.68rem]" /><AppSelect options={["All triggers", "Manual", "Webhook", "Redeploy", "Rollback"]} value={trigger} onValueChange={(value) => { setTrigger(value); resetPage() }} className="h-[2.1rem] min-w-0 bg-card px-2.5 text-[0.68rem]" /><Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className="h-[2.1rem] bg-card text-[0.68rem]"><FunnelSimpleIcon />Filters{advancedCount > 0 && <span className="grid size-4 place-items-center rounded-full bg-accent text-[9px] font-bold text-accent-foreground">{advancedCount}</span>}</Button></PopoverTrigger><PopoverContent align="start" sideOffset={7} className="w-[min(28rem,calc(100vw-2rem))] p-0"><header className="border-b bg-muted/70 px-4 py-3"><p className="text-xs font-semibold">Deployment filters</p><p className="mt-0.5 text-[10px] text-muted-foreground">Filters are applied by the server before pagination.</p></header><div className="grid gap-3 p-4 sm:grid-cols-2">{scope === "global" && <FilterField label="Application"><AppSelect aria-label="Filter by application" options={[{ value: all, label: "All applications" }, ...(applicationsQuery.data?.applications || []).map((item) => ({ value: item.id, label: item.name }))]} value={applicationFilter} onValueChange={(value) => { setApplicationFilter(value); setServiceFilter(all); setEnvironmentFilter(all); resetPage() }} disabled={applicationsQuery.isPending || applicationsQuery.isError} /></FilterField>}{scope !== "service" && <FilterField label="Service"><AppSelect aria-label="Filter by service" options={[{ value: all, label: "All services" }, ...services.map((item) => ({ value: item.id, label: item.name }))]} value={serviceFilter} onValueChange={(value) => { setServiceFilter(value); resetPage() }} disabled={!selectedApplicationID || applicationQuery.isPending || applicationQuery.isError} /></FilterField>}<FilterField label="Environment"><AppSelect aria-label="Filter by environment" options={[{ value: all, label: "All environments" }, ...environments.map((item) => ({ value: item.id, label: item.name }))]} value={environmentFilter} onValueChange={(value) => { setEnvironmentFilter(value); resetPage() }} disabled={!selectedApplicationID || applicationQuery.isPending || applicationQuery.isError} /></FilterField><FilterField label="Branch"><Input value={branch} onChange={(event) => { setBranch(event.target.value); resetPage() }} className="bg-card font-mono text-xs" placeholder="Exact branch" /></FilterField><FilterField label="From"><Input type="date" value={dateFrom} max={dateTo || undefined} onChange={(event) => { setDateFrom(event.target.value); resetPage() }} className="bg-card text-xs" /></FilterField><FilterField label="To"><Input type="date" value={dateTo} min={dateFrom || undefined} onChange={(event) => { setDateTo(event.target.value); resetPage() }} className="bg-card text-xs" /></FilterField>{(applicationsQuery.isError || applicationQuery.isError) && <p role="alert" className="sm:col-span-2 text-[10px] text-destructive">Application filter options could not be loaded. Deployment history remains available.</p>}</div><footer className="flex items-center justify-between border-t bg-muted/30 px-4 py-3"><span className="text-[10px] text-muted-foreground">{advancedCount} advanced filters active</span><Button size="sm" variant="ghost" disabled={!advancedCount} onClick={clearAdvanced}>Clear filters</Button></footer></PopoverContent></Popover><label className="relative min-w-0"><MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={14} /><Input value={query} onChange={(event) => setQuery(event.target.value)} className="hf-deployment-search h-[2.1rem] w-full min-w-0 bg-card px-2.5 text-[0.68rem]" placeholder="Search this result page" /></label></header>
        {scoped.length ? <div className="overflow-x-auto"><Table className="w-full min-w-[940px] text-left"><TableHeader><TableRow><TableHead>Deployment</TableHead><TableHead>Application / service</TableHead><TableHead>Environment</TableHead><TableHead>Commit</TableHead><TableHead>Status</TableHead><TableHead>URL</TableHead><TableHead>Trigger</TableHead><TableHead>Started</TableHead></TableRow></TableHeader><TableBody>{scoped.map((deployment) => <TableRow key={deployment.id}><TableCell><Link to={"/deployments/" + deployment.id} className="font-mono text-xs font-semibold hover:underline">{deployment.id}</Link></TableCell><TableCell><span className="text-[10px] text-muted-foreground">{deployment.application_name || deployment.application_id}</span><span className="block text-xs font-semibold">{deployment.service_name || deployment.service_id}</span></TableCell><TableCell><span className="text-xs">{deployment.environment_name || deployment.environment_id}</span><span className="block font-mono text-[10px] text-muted-foreground">{deployment.branch}</span></TableCell><TableCell className="font-mono text-xs">{deployment.commit_hash || "Pending"}</TableCell><TableCell><StatusPill status={deployment.status === "SUCCESS" ? "Healthy" : deployment.status[0] + deployment.status.slice(1).toLowerCase()} /></TableCell><TableCell>{deployment.public_url ? <a href={deployment.public_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold hover:underline">Visit<ArrowSquareOutIcon size={12} /></a> : <span className="text-[10px] text-muted-foreground">{deployment.is_active ? "No domain" : "—"}</span>}</TableCell><TableCell className="text-xs text-muted-foreground">{deployment.trigger}</TableCell><TableCell className="text-xs text-muted-foreground">{new Date(deployment.created_at).toLocaleString()}</TableCell></TableRow>)}</TableBody></Table></div> : <div className="px-6 py-16 text-center"><CloudArrowUpIcon className="mx-auto text-muted-foreground" size={24} /><p className="mt-3 text-sm font-semibold">No deployments found</p><p className="mt-1 text-xs text-muted-foreground">Deployments appear here after a configured service is deployed.</p></div>}
...
function FilterField({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block min-w-0"><span className="mb-1.5 block text-[10px] font-semibold text-muted-foreground">{label}</span>{children}</label>
}
  ...
  return (
    <main className="mx-auto w-full max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
      <div className="mb-7 flex flex-col gap-4 xl:flex-row xl:items-end">
        <div><div className="mb-2 flex items-center gap-2"><StatusPill status={displayStatus} /><span className="text-xs font-medium text-muted-foreground">{deployment.environment_name || deployment.environment_id}</span>{deployment.is_active && <StatusBadge tone="info">Active release</StatusBadge>}</div><h1 className="font-mono text-3xl font-semibold tracking-[-0.04em]">{deployment.id}</h1><p className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground"><span>{deployment.application_name}</span>{deployment.application_id && deployment.service_id ? <Link to={"/applications/" + deployment.application_id + "/services/" + deployment.service_id} className="font-semibold text-foreground hover:underline">{deployment.service_name || deployment.service_id}</Link> : <span className="font-mono">{deployment.service_id}</span>}<span>{deployment.trigger || "manual"}</span><span>{new Date(deployment.created_at).toLocaleString()}</span></p></div>
        <div className="flex flex-wrap gap-2 xl:ml-auto">
          {deployment.public_url && <Button asChild><a href={deployment.public_url} target="_blank" rel="noreferrer"><ArrowSquareOutIcon />Visit deployment</a></Button>}
          <Button variant="outline" disabled={!logText} onClick={downloadLogs}><DownloadSimpleIcon />Download logs</Button>
          <Button variant="outline" disabled={active || releaseMutationPending || !deployment.commit_hash} onClick={() => redeployMutation.mutate()}><ArrowsClockwiseIcon />{redeployMutation.isPending ? "Redeploying..." : "Redeploy commit"}</Button>
          {deployment.status === "SUCCESS" && <ConfirmationAction title="Roll back to this release?" description="HostForge will create a new auditable deployment from this exact commit. Current history remains unchanged, and cutover only occurs after the new release passes its health check." confirmLabel="Create rollback" onConfirm={() => rollbackMutation.mutate()} trigger={<Button variant="outline" disabled={releaseMutationPending}>Rollback to this release</Button>} />}
          {active && <ConfirmationAction title="Cancel this deployment?" description="The queued or running build will stop. Existing active releases are not changed." confirmLabel="Cancel deployment" onConfirm={() => cancelMutation.mutateAsync()} trigger={<Button disabled={releaseMutationPending}>Cancel deployment</Button>} />}
        </div>
      </div>
      ...
      {redeployMutation.isError && <div className="mb-5 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-xs text-destructive">The server could not redeploy this commit.</div>}
      ...
      <section className="mb-5 overflow-hidden rounded-xl border bg-card">
        <header className="flex flex-col gap-3 border-b bg-muted/75 p-4 xl:flex-row xl:items-center"><div className="flex items-center gap-2"><TerminalWindowIcon size={16} /><div><h2 className="text-sm font-semibold">Deployment logs</h2><p className="mt-0.5 text-[11px] text-muted-foreground">{active ? "Live build output — this view follows the deployment as it advances" : "Completed server log snapshot"}</p></div></div><div className="flex flex-wrap gap-2 xl:ml-auto"><label className="relative"><MagnifyingGlassIcon className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" size={13} /><Input value={logQuery} onChange={(event) => setLogQuery(event.target.value)} className="h-[2.1rem] w-56 min-w-0 bg-card pl-8 text-[0.68rem]" placeholder="Search logs" /></label><button type="button" aria-pressed={wrap} onClick={() => setWrap((current) => !current)} className={`hf-log-toggle ${wrap ? "hf-log-toggle-active" : ""}`}>Wrap</button><Button variant="outline" size="icon" aria-label="Copy log output" disabled={!logText} onClick={() => navigator.clipboard.writeText(logText)}><CopyIcon /></Button></div></header>
      ...
      <div className="mb-5 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
        ...
        <Panel title="Deployment information" subtitle="Server-recorded build and release data">
          <div className="divide-y">{[
            { label: "Application", value: deployment.application_name || deployment.application_id || "Unknown" },
            { label: "Service", value: deployment.service_name || deployment.service_id },
            { label: "Environment", value: deployment.environment_name || deployment.environment_id },
            ...(deployment.public_url ? [{ label: "Active URL", value: deployment.public_url }] : []),
