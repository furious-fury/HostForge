...
export function OnboardingScreen() {
  ...
  // GitHub sends the manifest code back as a query parameter and it is good
  // for exactly one exchange. Requiring a click to spend it meant landing
  // back here looked like nothing had happened, and navigating away or
  // refreshing lost the code silently -- leaving an App created on GitHub
  // that HostForge had no credentials for, recoverable only by starting
  // over. Spend it on arrival instead; the card below is now a retry for
  // when this fails, not the only way through.
  //
  // The ref guard is what keeps it to one attempt: a second exchange of the
  // same code always fails, so a re-render or React's double-invoked effects
  // in development must not re-fire it.
  const alreadyConfigured = Boolean(appQuery.data?.app.configured || onboardingQuery.data?.onboarding.github_app_complete)
  const exchangeRequested = useRef(false)
  const exchangeMutate = exchange.mutate
  useEffect(() => {
    if (!code || appQuery.isPending || alreadyConfigured || exchangeRequested.current) return
    exchangeRequested.current = true
    exchangeMutate(code)
  }, [code, appQuery.isPending, alreadyConfigured, exchangeMutate])
  ...
  return <main className="min-h-svh bg-background text-foreground"><header className="flex h-16 items-center border-b bg-card px-5 sm:px-8"><Link to="/" className="flex items-center gap-3"><span className="grid size-8 place-items-center rounded-lg bg-accent text-accent-foreground"><CubeIcon size={17} weight="fill" /></span><span className="text-sm font-semibold">HostForge</span></Link><Link to="/" className="ml-auto text-xs font-medium text-muted-foreground hover:text-foreground">Finish later</Link></header><div className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
    ...
    {code && !githubConfigured && <section className="mb-5 rounded-xl border bg-card p-5"><h2 className="text-sm font-semibold">{exchange.isError ? "GitHub App exchange failed" : "Connecting the GitHub App"}</h2><p className="mt-1 text-xs text-muted-foreground">{exchange.isError ? "The one-time manifest code could not be exchanged. It may already have been used, or it may have expired. Retrying below reuses the same code; if that fails, generate a new manifest and create the App again." : "GitHub returned a one-time manifest code. Exchanging it now to store encrypted App credentials."}</p>{exchange.isError && <Button className="mt-4" disabled={exchange.isPending} onClick={() => exchange.mutate(code)}>{exchange.isPending ? "Connecting..." : "Retry exchange"}</Button>}</section>}
