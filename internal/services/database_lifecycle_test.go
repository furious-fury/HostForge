...
func TestDeleteDatabaseServiceRetainsVolumeMetadata(t *testing.T) {
	...
	result, err := DeleteDatabaseServiceAndRuntime(ctx, log, store, created.Service.ID, "operator")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeleteDatabaseServiceRejectsRunningOperation(t *testing.T) {
	...
	_, err = DeleteDatabaseServiceAndRuntime(ctx, slog.New(slog.NewTextHandler(io.Discard, nil)), store, created.Service.ID, "operator")
