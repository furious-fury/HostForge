...
// ReplaceRoot atomically installs a complete root Caddyfile. If validation or
// reload fails, the previous file is restored and reloaded before returning.
func ReplaceRoot(ctx context.Context, caddyBin, rootConfig, content string) error {
	root := strings.TrimSpace(rootConfig)
	if root == "" {
		return fmt.Errorf("root config path is required")
	}
	previous, readErr := os.ReadFile(root)
	if readErr != nil && !os.IsNotExist(readErr) {
		return fmt.Errorf("read current root config: %w", readErr)
	}
	if err := writeAtomic(root, []byte(content)); err != nil {
		return err
	}
	...
	if err := ValidateRoot(ctx, bin, root); err != nil {
		if readErr == nil {
			_ = writeAtomic(root, previous)
		}
		return fmt.Errorf("validate replacement caddy config: %w", err)
	}
	if err := runCaddy(ctx, bin, "reload", "--config", root); err != nil {
		...
		return fmt.Errorf("reload replacement caddy config: %w", err)
	}
	return nil
}
...
// RenderPermanentControlPlaneConfig renders the post-bootstrap domain route.
func RenderPermanentControlPlaneConfig(domain, generatedPath string) string {
	return "{\n\tauto_https disable_redirects\n}\n\n# generated by hostforge onboarding\nhttps://" + strings.TrimSpace(domain) + " {\n\ttls\n\treverse_proxy 127.0.0.1:8080\n}\n\nimport " + strings.TrimSpace(generatedPath) + "\n"
}
